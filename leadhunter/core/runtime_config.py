...
class RuntimeConfig:
    ...
    Attributes:
        ...
        enabled_sources: Аварийный рубильник владельца. Пустой список (по
            умолчанию) = разрешены все, а выбирают биржи сами пользователи в
            боте. Перечислять их здесь нужно только чтобы принудительно
            запретить площадку всем сразу.
        llm: Настройки LLM-провайдера (по умолчанию локальный Ollama).
    """

    min_score: int = 60
    min_budget: int = 50
    # Пусто = без ограничения. Именно пусто, а не полный список: иначе новая
    # биржа в реестре оказалась бы «не перечисленной» в уже созданных
    # settings.yaml и молча не работала бы у всех, кто обновился.
...
# Биржи выбирает КАЖДЫЙ пользователь сам — в боте: ⚙️ Настройки → 🌐 Биржи.
# Этот список нужен только как аварийный рубильник владельца: перечисленные
# здесь площадки разрешены, остальные не опрашиваются ни у кого.
# Пустой список [] (по умолчанию) = ничего не запрещаем.
